...
export async function GET(request: Request) {
  ...
  try {
    const result = await api.get<WorkStepsHeaderShowResponse>(
      "/admin/home/steps/header",
      {
        locale,
        headers: {
    ...
    if (error instanceof ApiError) {
      return Response.json(
        {
          success: false,
          message: error.message || t.unableToFetchWorkStepsHeader,
          data: error.data,
        },
    ...
    console.error("Work steps header fetch failed:", error);

    return Response.json(
      { success: false, message: t.unableToFetchWorkStepsHeader },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json(
      { success: false, message: t.invalidWorkStepsHeaderForm },
      { status: 400 },
    );
  }
  ...
  if (!content || !title || !description) {
    return Response.json(
      { success: false, message: t.workStepsHeaderRequired },
      { status: 400 },
    );
  }

  try {
    const result = await api.post<WorkStepsHeaderMutationResponse>(
      "/admin/home/steps/header",
      {
        locale,
        headers: {
    ...
    if (error instanceof ApiError) {
      return Response.json(
        {
          success: false,
          message: error.message || t.unableToSaveWorkStepsHeader,
          data: error.data,
        },
        { status: error.status || 500 },
      );
    ...
    console.error("Work steps header save failed:", error);

    return Response.json(
      { success: false, message: t.unableToSaveWorkStepsHeader },
      { status: 500 },
    );
  }
}
