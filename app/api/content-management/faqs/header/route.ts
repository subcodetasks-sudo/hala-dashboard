...
export async function GET(request: Request) {
  ...
  try {
    const result = await api.get<FaqsHeaderShowResponse>(
      "/admin/home/faqs/header",
      {
        locale,
        headers: {
    ...
    if (error instanceof ApiError) {
      return Response.json(
        {
          success: false,
          message: error.message || t.unableToFetchFaqsHeader,
          data: error.data,
        },
    ...
    console.error("FAQs header fetch failed:", error);

    return Response.json(
      { success: false, message: t.unableToFetchFaqsHeader },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json(
      { success: false, message: t.invalidFaqsHeaderForm },
      { status: 400 },
    );
  }
  ...
  if (!content || !title || !description) {
    return Response.json(
      { success: false, message: t.faqsHeaderRequired },
      { status: 400 },
    );
  }

  try {
    const result = await api.post<FaqsHeaderMutationResponse>(
      "/admin/home/faqs/header",
      {
        locale,
        headers: {
    ...
    if (error instanceof ApiError) {
      return Response.json(
        {
          success: false,
          message: error.message || t.unableToSaveFaqsHeader,
          data: error.data,
        },
        { status: error.status || 500 },
      );
    ...
    console.error("FAQs header save failed:", error);

    return Response.json(
      { success: false, message: t.unableToSaveFaqsHeader },
      { status: 500 },
    );
  }
}
