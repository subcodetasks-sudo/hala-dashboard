...
import type { CancellationSourcesResponse } from "@/features/orders/types";
...
function getRouteMessages(locale: ReturnType<typeof normalizeApiLocale>) {
  return (locale === "en" ? enMessages : arMessages).Orders.Cancelled.route;
}
...
export async function GET(request: Request) {
  ...
  try {
    const result = await api.get<CancellationSourcesResponse>(
      "/admin/renewal-requests/cancellation-sources",
      {
        locale,
        headers: {
    ...
    console.error("Cancellation sources fetch failed:", error);
