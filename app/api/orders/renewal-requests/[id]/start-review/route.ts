...
import type { StartReviewResponse } from "@/features/orders/types";
...
function getRouteMessages(locale: ReturnType<typeof normalizeApiLocale>) {
  return (locale === "en" ? enMessages : arMessages).Orders.New.startReviewDialog
    .route;
}
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const result = await api.post<StartReviewResponse>(
      `/admin/renewal-requests/${encodeURIComponent(id.trim())}/start-review`,
      {
        locale,
        headers: {
