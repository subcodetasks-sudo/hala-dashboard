...
import type { HoldReasonsResponse } from "@/features/orders/types";
...
function getRouteMessages(locale: ReturnType<typeof normalizeApiLocale>) {
  return (locale === "en" ? enMessages : arMessages).Orders.Pending.route;
}
...
export async function GET(request: Request) {
  ...
  try {
    const result = await api.get<HoldReasonsResponse>(
      "/admin/renewal-requests/hold-reasons",
      {
        locale,
        headers: {
    ...
    console.error("Hold reasons fetch failed:", error);
