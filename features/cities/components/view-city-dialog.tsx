...
import { useCity } from "@/features/cities/queries/use-city";

type ViewCityDialogProps = {
  open: boolean;
...
export default function ViewCityDialog({
  open,
  onOpenChange,
  cityId,
}: ViewCityDialogProps) {
  const t = useTranslations("Cities.viewDialog");
  const { data: city, isLoading, isError, error, isFetching } = useCity(
    cityId,
    open && cityId != null,
  );
      ...
      >
        ...
        <div className="flex min-w-0 flex-col gap-3 px-5 py-5">
          {isLoading || isFetching ? (
            ...
            <>
              <DetailRow label={t("nameAr")}>{city.nameAr || "—"}</DetailRow>
              <DetailRow label={t("nameEn")}>{city.nameEn || "—"}</DetailRow>
