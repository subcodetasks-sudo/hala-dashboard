...
import { useDeleteIssuePlace } from "@/features/cities/queries/use-delete-issue-place";
import type { IssuePlaceRow } from "@/features/cities/types";

type DeleteIssuePlaceDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  place: IssuePlaceRow | null;
};

export default function DeleteIssuePlaceDialog({
  open,
  onOpenChange,
  place,
}: DeleteIssuePlaceDialogProps) {
  const t = useTranslations("Cities.issuePlaceDeleteDialog");
  const locale = useLocale();
  const deletePlace = useDeleteIssuePlace();
  const isPending = deletePlace.isPending;
