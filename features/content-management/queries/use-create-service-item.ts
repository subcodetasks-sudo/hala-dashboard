...
async function createServiceItem(
  ...
  const response = await fetch("/api/content-management/services", {
    method: "POST",
    headers: {
...
export function useCreateServiceItem() {
  ...
  return useMutation({
    mutationFn: (input: CreateServiceItemInput) =>
      createServiceItem(locale, input),
    onSuccess: () => {
