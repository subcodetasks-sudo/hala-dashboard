...
export type CreateStepItemInput = {
...
async function createStepItem(
  locale: string,
  input: CreateStepItemInput,
...
export function useCreateStepItem() {
  const locale = useLocale();
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (input: CreateStepItemInput) => createStepItem(locale, input),
    onSuccess: () => {
