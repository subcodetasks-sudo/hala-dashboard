...
/** Normalizes a statistics API item into a table row. */
export function mapStatisticToRow(
  entry: unknown,
  locale: AppLocale,
): StatisticItemRow | null {
  if (!isRecord(entry)) {
    return null;
  }
