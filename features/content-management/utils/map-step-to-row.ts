...
/** Normalizes a work-step API item into a table row. */
export function mapStepToRow(
  entry: unknown,
  locale: AppLocale,
): StepItemRow | null {
  if (!isRecord(entry)) {
  ...
  return {
    id,
    stepNumber,
    stepNameAr: stepName.ar,
    stepNameEn: stepName.en,
    titleAr: title.ar,
    titleEn: title.en,
