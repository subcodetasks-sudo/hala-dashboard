...
/** Normalizes a legal section API item into a table row. */
export function mapLegalSectionToRow(
  entry: unknown,
  locale: AppLocale,
): LegalSectionRow | null {
  if (!isRecord(entry)) {
    return null;
  }
  ...
  const content = readLocalizedText(entry, ["content"]);
