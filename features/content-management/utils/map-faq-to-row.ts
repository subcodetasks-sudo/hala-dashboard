...
/** Normalizes a FAQ API item into a table row. */
export function mapFaqToRow(
  entry: unknown,
  locale: AppLocale,
): FaqItemRow | null {
  if (!isRecord(entry)) {
  ...
  const question = readLocalizedText(entry, ["question"]);
  const answer = readLocalizedText(entry, ["answer"]);
