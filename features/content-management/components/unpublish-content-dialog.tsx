...
import { useUnpublishContent } from "@/features/content-management/queries/use-unpublish-content";

type UnpublishContentDialogProps = {
  open: boolean;
...
export default function UnpublishContentDialog({
  open,
  onOpenChange,
  contentId,
  title,
}: UnpublishContentDialogProps) {
  const t = useTranslations("ContentManagement.unpublishDialog");
  const unpublish = useUnpublishContent();
  const isPending = unpublish.isPending;
  ...
  const handleConfirm = () => {
    if (isPending || !contentId) return;

    unpublish.mutate(contentId, {
      onSuccess: () => {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        {/* RTL: first item renders on the right (Confirm → Cancel). */}
