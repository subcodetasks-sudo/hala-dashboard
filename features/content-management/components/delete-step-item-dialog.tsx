...
import { useDeleteStepItem } from "@/features/content-management/queries/use-delete-step-item";
import type { StepItemRow } from "@/features/content-management/types";

type DeleteStepItemDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  item: StepItemRow | null;
};

export default function DeleteStepItemDialog({
  open,
  onOpenChange,
  item,
}: DeleteStepItemDialogProps) {
  const t = useTranslations("ContentManagement.workSteps.items.deleteDialog");
  const locale = useLocale();
  const deleteItem = useDeleteStepItem();
      ...
      >
        ...
        <div className="flex min-w-0 flex-col items-center gap-4 px-5 py-6 text-center">
          <div className="flex size-16 items-center justify-center rounded-full bg-brand-accent/15">
            <CustomIcon src="/svg/trash.svg" size={28} className="text-brand-accent" />
          </div>
          ...
          <div className="flex w-full min-w-0 items-center justify-between gap-3 rounded-2xl bg-brand-background px-4 py-3 text-start">
            <div className="flex min-w-0 items-center gap-2">
              <CustomIcon src="/svg/routing-2.svg" size={18} className="shrink-0 text-brand-gris" />
              <span className="shrink-0 text-sm font-medium text-brand-gris">{t("itemLabel")}</span>
            </div>
            <span className="min-w-0 truncate text-sm font-bold text-brand-black">{displayName}</span>
          </div>
        </div>
            ...
            {isPending ? (
              ...
              <>
                <span className="confirm-chevron-start inline-flex items-center" aria-hidden>
                  <ChevronsLeft className="size-4 transition-transform duration-300 group-hover:-translate-x-0.5 ltr:rotate-180" strokeWidth={2.25} />
                </span>
                <span className="tracking-wide">{t("confirm")}</span>
                <span className="confirm-chevron-end inline-flex items-center" aria-hidden>
                  <ChevronsRight className="size-4 transition-transform duration-300 group-hover:translate-x-0.5 ltr:rotate-180" strokeWidth={2.25} />
                </span>
              </>
            )}
          </Button>
