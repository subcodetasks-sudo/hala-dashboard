...
import { useCreateStatisticItem } from "@/features/content-management/queries/use-create-statistic-item";
import { useUpdateStatisticItem } from "@/features/content-management/queries/use-update-statistic-item";
import {
  createStatisticItemFormSchema,
  EMPTY_STATISTIC_ITEM_FORM_VALUES,
} from "@/features/content-management/schemas/statistic-item-form-schema";
import type {
...
type StatisticItemFormDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  item?: StatisticItemRow | null;
};
...
export default function StatisticItemFormDialog({
  open,
  onOpenChange,
  item = null,
}: StatisticItemFormDialogProps) {
  const t = useTranslations("ContentManagement.statistics.items.formDialog");
  const createItem = useCreateStatisticItem();
  const updateItem = useUpdateStatisticItem();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<StatisticItemFormValues>({
    resolver: zodResolver(schema),
    defaultValues: EMPTY_STATISTIC_ITEM_FORM_VALUES,
  });
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    if (!nextOpen) {
      if (isPending) return;
      clearLocalPreview();
      reset(EMPTY_STATISTIC_ITEM_FORM_VALUES);
    }
      ...
      >
        ...
        >
          <div className="flex flex-col gap-1.5">
            <Label
              htmlFor="statistic-description-ar"
          ...
          <div className="flex flex-col gap-1.5">
            <Label
              htmlFor="statistic-description-en"
              className="text-sm font-semibold text-brand-black"
            >
