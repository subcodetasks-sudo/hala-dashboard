...
import { useServicesHeader } from "@/features/content-management/queries/use-services-header";
import { useUpsertServicesHeader } from "@/features/content-management/queries/use-upsert-services-header";
import SectionHeaderContentTabs from "@/components/section-header-content-tabs";
import ServicesItemsSection from "@/features/content-management/components/services-items-section";
import {
  createServicesHeaderFormSchema,
  EMPTY_SERVICES_HEADER_FORM_VALUES,
} from "@/features/content-management/schemas/services-header-form-schema";
import type {
...
function toFormValues(
  header: ServicesHeaderApiItem | null | undefined,
): ServicesHeaderFormValues {
  if (!header) return EMPTY_SERVICES_HEADER_FORM_VALUES;
...
export default function ServicesHeaderView() {
  const t = useTranslations("ContentManagement.services");
  ...
  const { data: header, isLoading, isError, error, refetch } =
    useServicesHeader();
  const upsertHeader = useUpsertServicesHeader();
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<ServicesHeaderFormValues>({
...
function ServicesHeaderSkeleton() {
  return (
    <div
