...
import { useWorkStepsHeader } from "@/features/content-management/queries/use-work-steps-header";
import { useUpsertWorkStepsHeader } from "@/features/content-management/queries/use-upsert-work-steps-header";
import SectionHeaderContentTabs from "@/components/section-header-content-tabs";
import WorkStepsItemsSection from "@/features/content-management/components/work-steps-items-section";
import {
  createWorkStepsHeaderFormSchema,
  EMPTY_WORK_STEPS_HEADER_FORM_VALUES,
} from "@/features/content-management/schemas/work-steps-header-form-schema";
import type {
...
function toFormValues(
  header: WorkStepsHeaderApiItem | null | undefined,
): WorkStepsHeaderFormValues {
  if (!header) return EMPTY_WORK_STEPS_HEADER_FORM_VALUES;
...
export default function WorkStepsHeaderView() {
  const t = useTranslations("ContentManagement.workSteps");
  ...
  const { data: header, isLoading, isError, error, refetch } =
    useWorkStepsHeader();
  const upsertHeader = useUpsertWorkStepsHeader();
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<WorkStepsHeaderFormValues>({
  ...
  if (isLoading) {
    return <WorkStepsHeaderSkeleton />;
  }
...
function WorkStepsHeaderSkeleton() {
  return (
    <div
