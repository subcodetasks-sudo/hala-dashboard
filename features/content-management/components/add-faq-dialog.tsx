...
import { useAddFaq } from "@/features/content-management/queries/use-add-faq";
import { createAddFaqSchema } from "@/features/content-management/schemas/add-faq-schema";
import type { AddFaqFormValues } from "@/features/content-management/types";
...
export default function AddFaqDialog({
  ...
  const t = useTranslations("ContentManagement.addFaqDialog");
  ...
  const schema = useMemo(
    () =>
      createAddFaqSchema({
        questionRequired: t("validation.questionRequired"),
        answerRequired: t("validation.answerRequired"),
        appearanceRequired: t("validation.appearanceRequired"),
        displayOrderRequired: t("validation.displayOrderRequired"),
  ...
  const isPending = addFaq.isPending;
  ...
  const submitWithStatus = (status: AddFaqFormValues["status"]) =>
    handleSubmit((values) => {
      addFaq.mutate(
        { ...values, status },
        {
...
<div className="border-b border-black/10 w-9/10 mx-auto"></div>
        <form
                  ...
                  >
                    ...
                    <SelectContent className="rounded-2xl border-brand-primary/20">
                      <SelectItem value="homePage">
                        {t("appearance.homePage")}
                      </SelectItem>
                      <SelectItem value="supportPage">
                        {t("appearance.supportPage")}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                )}
