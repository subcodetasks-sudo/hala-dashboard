...
import LegalPagePanel from "@/features/content-management/components/legal-page-panel";
import type { LegalPageKind } from "@/features/content-management/types";
import { useRouter } from "@/i18n/navigation";
import { cn } from "@/lib/utils";

const LEGAL_TABS = [
  { id: "terms" as const, iconSrc: "/svg/lamp-charge.svg" },
  { id: "privacy" as const, iconSrc: "/svg/info-circle.svg" },
] as const;
...
export default function LegalPagesView() {
  ...
  const [activeTab, setActiveTab] = useState<LegalPageKind>("terms");
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 p-4 pb-8">
      ...
      <Tabs
        value={activeTab}
        onValueChange={(value) => setActiveTab(value as LegalPageKind)}
      ...
      >
        <TabsList className="flex h-auto w-full flex-wrap gap-3 rounded-none bg-transparent p-0 group-data-horizontal/tabs:h-auto!">
          {LEGAL_TABS.map((tab, index) => (
            <TabsTrigger
              ...
              className={cn(
                TAB_TRIGGER_CLASS,
                tabRoundedClass(index, LEGAL_TABS.length),
              )}
            >
              <CustomIcon
                src={tab.iconSrc}
        ...
        <TabsContent value="terms" className="mt-0">
          <LegalPagePanel page="terms" />
        </TabsContent>
        <TabsContent value="privacy" className="mt-0">
          <LegalPagePanel page="privacy" />
        </TabsContent>
      </Tabs>
    </div>
  );
