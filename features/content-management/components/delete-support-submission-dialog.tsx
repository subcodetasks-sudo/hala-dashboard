...
import { useDeleteSupportSubmission } from "@/features/content-management/queries/use-delete-support-submission";
import type { SupportSubmissionRow } from "@/features/content-management/types";

type DeleteSupportSubmissionDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  item: SupportSubmissionRow | null;
};

export default function DeleteSupportSubmissionDialog({
  open,
  onOpenChange,
  item,
}: DeleteSupportSubmissionDialogProps) {
  const t = useTranslations(
    "ContentManagement.support.submissions.deleteDialog",
  );
  const deleteItem = useDeleteSupportSubmission();
  const isPending = deleteItem.isPending;

  const displayName = item?.fullName || item?.phone || "";
