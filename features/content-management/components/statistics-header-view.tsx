...
import { useStatisticsHeader } from "@/features/content-management/queries/use-statistics-header";
import { useUpsertStatisticsHeader } from "@/features/content-management/queries/use-upsert-statistics-header";
import SectionHeaderContentTabs from "@/components/section-header-content-tabs";
import StatisticsItemsSection from "@/features/content-management/components/statistics-items-section";
import {
  createStatisticsHeaderFormSchema,
  EMPTY_STATISTICS_HEADER_FORM_VALUES,
} from "@/features/content-management/schemas/statistics-header-form-schema";
import type {
...
function toFormValues(
  header: StatisticsHeaderApiItem | null | undefined,
): StatisticsHeaderFormValues {
  if (!header) return EMPTY_STATISTICS_HEADER_FORM_VALUES;
...
export default function StatisticsHeaderView() {
  const t = useTranslations("ContentManagement.statistics");
  ...
  const { data: header, isLoading, isError, error, refetch } =
    useStatisticsHeader();
  const upsertHeader = useUpsertStatisticsHeader();
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<StatisticsHeaderFormValues>({
  ...
  if (isLoading) {
    return <StatisticsHeaderSkeleton />;
  }
    ...
    return (
      <div className="flex min-w-0 flex-col gap-6 p-4 pb-8">
        ...
        <SectionHeaderContentTabs
          ...
          content={<StatisticsItemsSection />}
        />
      </div>
    );
  }
