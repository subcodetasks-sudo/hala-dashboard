...
import { useDeleteCancelStatus } from "@/features/cancel-statuses/queries/use-delete-cancel-status";
import type { CancelStatusRow } from "@/features/cancel-statuses/types";

type DeleteCancelStatusDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  item: CancelStatusRow | null;
};

export default function DeleteCancelStatusDialog({
  open,
  onOpenChange,
  item,
}: DeleteCancelStatusDialogProps) {
  const t = useTranslations("CancelStatuses.deleteDialog");
  const locale = useLocale();
  const deleteItem = useDeleteCancelStatus();
